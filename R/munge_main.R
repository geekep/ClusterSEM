...
Please note that this is likely effective sample size cut in half. The function is automatically doubling this value. This should only be used for liability h^2 conversion for binary traits and that it should reflect the sum of effective sample sizes across cohorts.", file = log.file)
  ...
  hold_names <- .get_renamed_colnames(toupper(names(file)), column.names,
                                      c("P", "A1", "A2", "effect", "SNP"),
                                      filename, N_provided = N_provided,
  ...
  b <- nrow(file)
  if("P" %in% colnames(file)) {file <- subset(file, !(is.na(file$P)))}
  ...
  b <- nrow(file)
  if("effect" %in% colnames(file)) {file <- subset(file, !(is.na(file$effect)))}
  ...
  a2 <- file$effect[[1]]
  if (a1 != a2)
    .LOG("The effect column was determined to be coded as an odds ratio (OR) for the ", filename,
         " summary statistics file. Please ensure this is correct.", file = log.file)
  ...
  if(!("N" %in% names(file)) & (exists("N") == FALSE))
    .LOG('Cannot find sample size column for', filename, " and a sample size was not provided for the N argument. Please either provide a total sample size to the N argument or try changing the name of the sample size column to N.", file = log.file)
