#' Use 'Parallel' R package to parallelly estimate a genetic covariance structure using multivariate LD score regression
...
#' 
#' @export
ldsc_parallel <- function(traits, sample.prev, population.prev, ld, wld,
  ...
  readSumstat <- function(chi1) {
    ...
    cat("Read in summary statistics [", s <<- s + 1, "/", n.traits, "] from: ", chi1, '\n')
    ...
    cat("Out of ", nrow(y1), " SNPs, ", nrow(merged), " remain after merging with LD-score files", '\n')
    ...
    cat("Removing ", sum(rm), " SNPs with Chi^2 > ", chisq.max, "; ", nrow(merged), " remain.", '\n')
  ...
  ## Default to PSOCK cluster and it's faster when not copying large amounts of data
  cl <- parallel::makeCluster(cores, type="PSOCK")
  # parallel::clusterEvalQ(cl, '.LOG')
  # parallel::clusterExport(cl, '.LOG', envir=environment())
  all_y <- parallel::parLapply(cl=cl, X=traits, fun=readSumstat)
  ...
  for(j in 1:n.traits) {
    ...
    ## .ldsc_main functioin
    .ldsc_main <- function(k) {
      
      ##### HERITABILITY code
      if(j == k) {
        
        # .LOG("     ", "     ", file=log.file, print=FALSE)
        # .LOG("Estimating heritability [", s, "/", n.V, "] for a trait: ", chi1, file=log.file)
        cat("Estimating heritability [", s <- s + 1, "/", n.V, "] for a trait: ", chi1)
    ...
    .LOG('\n', "Parallelly run .ldsc_main:", file=log.file)
    ...
    # parallel::clusterEvalQ(cl, '.LOG')
    # parallel::clusterExport(cl, '.LOG', envir=environment())
    parallel::parLapply(cl=cl, X=(j:n.traits), fun=.ldsc_main)
    parallel::stopCluster(cl)
    .LOG("All ldsc run completely.\n", file=log.file)
