#' Run user-specified structural equation model on genetic covariance matrix
#' @description Function to run a user-specified structural equation model based on output from multivariate LDSC, HDL and metaCCA
#'
#' @param covstruc genetic covariance matrix
#' @param estimation Options are either Diagonally Weighted Least Squares ("DWLS"; the default) or Maximum Likelihood ("ML")
#' @param model Model to be specified using lavaan syntax
...
#' @param imp_cov Optional argument to denote whether the user wants the model-implied and residual covariance matrix included in the usermodel output (default = TRUE)
...
usermodel <-
  function(covstruc,
    ...
    test <-
      c(
        stringr::str_detect(model, "~"),
        stringr::str_detect(model, "="),
        stringr::str_detect(model, "\\+")
      )
    if (any(test) != TRUE) {
      warning(
        "Your model name may be listed in quotes; please remove the quotes and try re-running if the function has returned an error about not locating the ReorderModel."
      )
    }
    ...
    y <-
      y[!duplicated(apply(y, 1, function(x)
    ...
    for (i in 1:length(S_names)) {
      ...
      if (b == FALSE) {
        ...
        remove2[w] <- i
        V_LD <-
          V_LD[-grep(pattern = remove[1], row.names(V_LD)), -grep(pattern = remove[1], colnames(V_LD))]
        w <- w + 1
        if (!(is.matrix(V_LD))) {
    ...
    ##re-define k and z and model names after removing non-used variables
    k <- ncol(S_LD)
    z <- (k * (k + 1)) / 2
    ...
    S_LDb <- S_LD
    smooth1 <-
      ifelse(eigen(S_LD)$values[nrow(S_LD)] <= 0, S_LD <-
               as.matrix((Matrix::nearPD(S_LD, corr = FALSE))$mat), S_LD <- S_LD)
    LD_sdiff <- max(abs(S_LD - S_LDb))
    
    V_LDb <- V_LD
    smooth2 <-
      ifelse(eigen(V_LD)$values[nrow(V_LD)] <= 0, V_LD <-
               as.matrix((Matrix::nearPD(V_LD, corr = FALSE))$mat), V_LD <- V_LD)
    ...
    if (CFIcalc == TRUE) {
      ...
      write.null <- function(k,
                             label = "V",
                             label2 = "VF") {
        Model3 <- ""
        for (p in 1:k) {
          linestart3 <-
            paste(colnames(S_LD)[p], " ~~ ", colnames(S_LD)[p], sep = "")
        ...
        for (p in 1:k) {
          linestart2 <-
            paste(label2, p, " =~ 1*", colnames(S_LD)[p], sep = "")
        ...
        for (i in 1:(k - 1)) {
          linestartc <-
            paste(colnames(S_LD)[i], " ~~ 0*", colnames(S_LD)[i + 1],  sep = "")
          if (k - i >= 2) {
            linemidc <- ""
            for (j in (i + 2):k) {
          ...
          Modelsat <-
            paste(Modelsat, linestartc, linemidc, " \n ", sep = "")
        }
        
        ModelsatF <- ""
        for (i in 1:(k - 1)) {
          linestartc <-
            paste(" ", label2, i, " ~~ 0*", label2, i + 1,  sep = "")
          if (k - i >= 2) {
            linemidc <- ""
            for (j in (i + 2):k) {
          ...
          ModelsatF <-
            paste(ModelsatF, linestartc, linemidc, " \n ", sep = "")
        }
      ...
      ##create independent model for calculation of CFI
    ...
    if (class(empty3$value) != "lavaan") {
      warning(
        paste(
          "The function has stopped due to convergence issues for your primary model. Please contact us with your specific model and variables used or try specifying an alternative model"
        )
      )
    }
    ...
    empty4$warning$message[1] <-
      ifelse(
        is.null(empty4$warning$message),
        empty4$warning$message[1] <- 0,
        empty4$warning$message[1]
      )
    
    if (fix_resid == TRUE) {
      if (class(empty4$value)[1] == "simpleError" |
          lavaan::inspect(Model1_Results, "converged") == FALSE) {
        ...
        for (p in 1:k) {
          linestart3a <-
            paste(colnames(S_LD)[p],
                  ...
                  sep = "")
          linestart3b <-
            paste(paste(n[, p], collapse = ""), " > .0001", sep = "")
          Model3 <-
            paste(Model3, " \n ", linestart3a, " \n ", linestart3b, " \n ", sep = "")
        }
        ...
        if (class(empty4$value)[1] != "lavaan") {
          if (grepl("duplicate", as.character(empty4$value)[1]) == TRUE) {
            ...
            print(
              "The model as initially specified failed to converge. A lower bound of 0 on residual variances was automatically added to try and troubleshoot this. This behavior can be toggled off by setting the fix_resid argument to FALSE."
            )
          }
        } else{
          print(
    ...
    bread2 <-
      .tryCatch.W.E(bread <- solve(t(S2.delt) %*% S2.W %*% S2.delt, tol = toler))
    ...
    if (class(bread2$value)[1] != "matrix") {
      warning(
        "Error: The primary model did not converge! Additional warnings or errors are likely being printed by lavaan.
      ...
      check <- 1
      unstand <-
        data.frame(lavaan::inspect(Model1_Results, "list")[, c("lhs", "op", "rhs", "free", "est")])
    ...
    if (class(bread2$value)[1] == "matrix") {
      ...
      constraints <-
        subset(Model_Output$label, Model_Output$label != "")
      ...
      if (estimation == "DWLS") {
        if (":=" %in% Model_Output$op & !(is.na(SE[1]))) {
          ...
          x <-
            lavaan::lav_model_get_parameters(lavmodel, type = "free")
          ...
          #pull the ghost parameter point estimate
          ghost <-
            subset(Model_Output, Model_Output$op == ":=")[, c("lhs", "op", "rhs", "free", "label", "est")]
          
          ##combine with delta method SE
          ghost2 <- cbind(ghost, se.ghost)
          colnames(ghost2)[7] <- "Ghost_Unstandardized_SE"
          
        } else{
          se.ghost <- NA
          if (":=" %in% Model_Output$op & is.na(se.ghost[1])) {
            se.ghost <-
              rep("SE could not be computed",
                  dplyr::count(":=" %in% Model_Output$op)$freq)
            ghost <-
              subset(Model_Output, Model_Output$op == ":=")[, c("lhs", "op", "rhs", "free", "label", "est")]
            ghost2 <- cbind(ghost, se.ghost)
            colnames(ghost2)[7] <- "Ghost_Unstandardized_SE"
      ...
      if (r > 1) {
        empty <-
          .tryCatch.W.E(check <-
                          gdata::lowerTriangle(lavaan::inspect(Model1_Results, "cor.lv")[1:r, 1:r]))
        t <- max(check)
        t2 <- min(check)
      } else{
        t <- 1
      ...
      if (t > 1 | t2 < -1  | t == "NaN") {
        print(
          "Error: The primary model produced correlations among your latent variables that are either greater than 1 or less than -1, or the latent variables have negative variances.
        ...
        if (CFIcalc == TRUE) {
          ...
          testCFI$warning$message[1] <-
            ifelse(
              is.null(testCFI$warning$message),
              testCFI$warning$message[1] <- "Safe",
              testCFI$warning$message[1]
            )
          testCFI$warning$message[1] <-
            ifelse(
              is.na(lavaan::inspect(fitCFI, "se")$theta[1, 2]) == TRUE,
              testCFI$warning$message[1] <-
          ...
          if (as.character(testCFI$warning$message)[1] != "lavaan WARNING: model has NOT converged!") {
            ...
            testCFI2$warning$message[1] <-
              ifelse(
                is.null(testCFI2$warning$message),
                testCFI2$warning$message[1] <- "Safe",
                testCFI2$warning$message[1]
              )
            testCFI2$warning$message[1] <-
              ifelse(
                is.na(lavaan::inspect(ModelQ_Results_CFI , "se")$theta[1, 2]) == TRUE,
                testCFI2$warning$message[1] <-
                  "lavaan WARNING: model has NOT converged!",
                testCFI2$warning$message[1]
              )
            ...
            if (as.character(testCFI2$warning$message)[1] != "lavaan WARNING: model has NOT converged!") {
              #pull the delta matrix (this doesn't depend on N)
              S2.delt_Q_CFI <-
                lavaan::inspect(ModelQ_Results_CFI, "delta")
              
              ##weight matrix from stage 2
              S2.W_Q_CFI <-
                lavaan::inspect(ModelQ_Results_CFI, "WLS.V")
              
              #the "bread" part of the sandwich is the naive covariance matrix of parameter estimates that would only be correct if the fit function were correctly specified
              bread_Q_CFI <-
                solve(t(S2.delt_Q_CFI) %*% S2.W_Q_CFI %*% S2.delt_Q_CFI)
              ...
              #ohm-hat-theta-tilde is the corrected sampling covariance matrix of the model parameters
              Ohtt_Q_CFI <-
                bread_Q_CFI %*% t(lettuce_Q_CFI) %*% V_Reorder2 %*% lettuce_Q_CFI %*% bread_Q_CFI
              ...
              eta_test_CFI <-
                subset(eta_test_CFI, eta_test_CFI$free != 0)
              eta_CFI <- cbind(eta_test_CFI[, 14])
              
              #Combining all the pieces from above:
              Q_CFI <-
                t(eta_CFI) %*% P1_CFI %*% solve(Eig_CFI) %*% t(P1_CFI) %*% eta_CFI
            } else{
          ...
          if (!(is.character(Q_CFI)) & !(is.character(Q))) {
            ...
            CFI <-
              "Either the chi-square or null (i.e. independence) model did not converge"
          }
          
        }
        
        print("Calculating Standardized Results")
        ##transform the covariance matrix S_LD to S correlation matrix S_Stand
        ...
        bread_stand2 <-
          .tryCatch.W.E(bread_stand <-
                          solve(t(delt_stand) %*% W_stand %*% delt_stand, tol = toler))
        
        if (class(bread_stand2$value)[1] != "matrix" |
            lavaan::inspect(Fit_stand, "converged") == FALSE |
            class(emptystand)[1] == "simpleError") {
          warning(
            "The standardized model failed to converge. This likely indicates more general problems with the model solution. Unstandardized results are printed below but this should be interpreted with caution."
          )
          ...
          Ohtt_stand <-
            bread_stand %*% t(lettuce_stand) %*% Vcov_stand %*% lettuce_stand %*% bread_stand
          ...
          if (estimation == "DWLS") {
            ...
            if (":=" %in% Model_Stand$op & !(NA %in% Model_Stand$se)) {
              ...
              #pull the ghost parameter point estimate
              ghost_stand <-
                subset(Model_Stand,  Model_Stand$op == ":=")[, c("lhs", "op", "rhs", "free", "label", "est")]
              ...
              #vector of indirect effect derivatives evaluated @ parameter estimates
              Jac <-
                lavaan::lav_func_jacobian_complex(func = func, x = x)
              #replace vcov here with our corrected one. this gives parameter variance
              var.ind <- Jac %*% vcov %*% t(Jac)
              #square root of parameter variance = parameter SE.
              ...
              colnames(ghost2_stand)[7] <- "Ghost_Standardized_SE"
            } else{
              if (":=" %in% Model_Stand$op & (NA %in% Model_Stand$se)) {
                ...
                se.ghost_stand <-
                  rep("SE could not be computed",
                      dplyr::count(":=" %in% Model_Stand$op)$freq)
          ...
          stand <-
            data.frame(lavaan::inspect(Fit_stand, "list")[, c("free", "est")])
          ...
          if (!(is.character(Q))) {
            chisq <- Q
            AIC <-
              (Q + 2 * lavaan::inspect(Model1_Results, "fit")["npar"])
          } else{
            chisq <- Q
            AIC <- NA
          }
          ...
          base_model <-
            data.frame(lavaan::inspect(ReorderModel, "list")[, c("lhs", "op", "rhs", "free", "est")])
          base_model <-
            subset(base_model,!(
              paste0(base_model$lhs, base_model$op, base_model$rhs) %in% paste0(unstand2$lhs, unstand2$op, unstand2$rhs)
            ))
          base_model <-
            subset(base_model,
                   base_model$op == "=~" |
          ...
          std_all <-
            subset(
              std_all,
              paste0(std_all$lhs, std_all$op, std_all$rhs) %in% paste0(results$lhs, results$op, results$rhs)
            )
          ...
          results <-
            suppressWarnings(merge(results, std_all, by = "order", all = T))
          results$est.std <-
            ifelse(is.na(results$est.std),
                   results$est_stand,
                   results$est.std)
    ...
    if (class(bread2$value)[1] == "matrix" & check == 2) {
      ...
      if (!(is.character(modelfit$chisq)) &
          !(is.factor(modelfit$chisq))) {
        modelfit$chisq <- as.numeric(as.character(modelfit$chisq))
        modelfit$df <- as.numeric(as.character(modelfit$df))
        modelfit$p_chisq <-
          ifelse(
            !(is.character(modelfit$chisq)),
            modelfit$p_chisq <-
              pchisq(modelfit$chisq, modelfit$df, lower.tail = FALSE),
            modelfit$p_chisq <- NA
          )
        modelfit$chisq <-
          ifelse(modelfit$df == 0, is.na(modelfit$chisq), modelfit$chisq)
        modelfit$AIC <-
          ifelse(modelfit$df == 0, is.na(modelfit$AIC), modelfit$AIC)
        modelfit$p_chisq <-
          ifelse(modelfit$df == 0,
                 is.na(modelfit$p_chisq),
                 modelfit$p_chisq)
        modelfit$SRMR <-
          ifelse(modelfit$df == 0, is.na(modelfit$SRMR), modelfit$SRMR)
        if (CFIcalc == TRUE) {
          order <- c(1, 2, 6, 3, 4, 5)
          ...
          if (!(is.factor(modelfit$CFI))) {
            if (modelfit$CFI < 0) {
              warning(
                paste(
                  "CFI estimates below 0 should not be trusted, and indicate that the other model fit estimates should be interpreted with caution. A negative CFI estimates typically appears due to negative residual variances."
                )
              )
            }
          }
          modelfit$CFI <-
            ifelse(modelfit$df == 0, is.na(modelfit$CFI), modelfit$CFI)
        } else{
          order <- c(1, 2, 5, 3, 4)
          modelfit <- modelfit[, order]
        }
      ...
      if (modelfit$df == 0) {
        print(
          "Model fit statistics are all printed as NA as you have specified a fully saturated model (i.e., df = 0)"
        )
      }
      ...
      if (LD_sdiff > 0) {
        print(
          paste(
            "The S matrix was smoothed prior to model estimation due to a non-positive definite matrix. The largest absolute difference in a cell between the smoothed and non-smoothed matrix was ",
            LD_sdiff,
            "As a result of the smoothing, the largest Z-statistic change for the genetic covariances was ",
            Z_diff,
      ...
      if (LD_sdiff2 > 0) {
        print(
          paste(
            "The V matrix was smoothed prior to model estimation due to a non-positive definite matrix. The largest absolute difference in a cell between the smoothed and non-smoothed matrix was ",
            LD_sdiff2,
            "As a result of the smoothing, the largest Z-statistic change for the genetic covariances was ",
            Z_diff,
            ". We recommend setting the smooth_check argument to true if you are going to run a multivariate GWAS.",
      ...
      if (any(constraints2 == TRUE)) {
        print(
          "Please note that when equality constraints are used in the current version that the standardized output will also impose the same constraint."
        )
      }
      results$p_value <-
        2 * pnorm(abs(
          as.numeric(results$Unstand_Est) / as.numeric(results$Unstand_SE)
        ), lower.tail = FALSE)
      results$p_value <-
        ifelse(results$p_value == 0, "< 5e-300", results$p_value)
      
      if (empty4$warning$message[1] != 0 &
          !grepl("not recommended for continuous data",
                 empty4$warning$message[1])) {
        warning(
          paste0(
            "The unstandardized model produced the following warning: ",
            empty4$warning$message[1],
            sep = ""
          )
        )
      ...
      if (emptystand$warning$message[1] != 0 &
        ...
        warning(
          paste0(
            "The standardized model produced the following warning: ",
            emptystand$warning$message[1],
            sep = ""
          )
        )
      ...
      if (imp_cov == TRUE) {
        ...
        names(resid_cov) <-
          c(
            ...
            "Residual Covariance Matrix (i.e. Calculated_Observed_Cov - Model_Implied_Cov)"
          )
        return(list(
          modelfit = modelfit,
          results = results,
