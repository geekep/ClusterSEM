...
#' @export
usermodel <-function(covstruc, estimation="DWLS", model = "", CFIcalc=TRUE, std.lv=FALSE, imp_cov=TRUE, fix_resid=TRUE, toler=NULL) {
  
  time <- proc.time()
  ##determine if the model is likely being listed in quotes and print warning if so
  test <-
    c(stringr::str_detect(model, "~"),
  ...
  ##k = number of phenotype (i.e., number of columns in LD portion of S matrix)
  k <- ncol(S_LD)
  ...
  r <- nrow(lavaan::lavInspect(ReorderModel, "cor.lv"))
  ...
  if(fix_resid == TRUE) {
    if (class(empty4$value)[1] == "simpleError" |
        lavaan::lavInspect(Model1_Results, "converged") == FALSE) {
  ...
  if (!(is.null(empty4$warning))) {
    if (lavaan::lavInspect(Model1_Results, "converged") == FALSE) {
      warning("The model failed to converge on a solution. Please try specifying an alternative model.")
    }
  }
  ...
  if(class(bread2$value)[1] == "matrix"){
    ...
    if(estimation == "DWLS"){
      if(":=" %in% Model_Output$op & !(is.na(SE[1]))){
        ...
        vcov <- lavaan::lavInspect(Model1_Results, "vcov") 
    ...
    if (r > 1) {
      empty <-
        .tryCatch.W.E(check <-
                        gdata::lowerTriangle(lavaan::lavInspect(Model1_Results, "cor.lv")[1:r, 1:r]))
      t <- max(check)
      t2 <- min(check)
    } else{
      t <- 1
    ...
    if(t > 1 | t2 < -1  | t == "NaN") {
      ...
      implied <- as.matrix(lavaan::fitted(Model1_Results))[1]
      ...
      if(class(bread_stand2$value)[1] != "matrix" | lavaan::lavInspect(Fit_stand,"converged") == FALSE | class(emptystand)[1] == "simpleError"){
        ...
        if(estimation == "DWLS"){
          ...
          if(":=" %in% Model_Stand$op & !(NA %in% Model_Stand$se)){
            #variance-covariance matrix of parameter estimates, q-by-q (this is the naive one)
            vcov <- lavaan::lavInspect(Fit_stand, "vcov") 
        ...
        if(CFIcalc == TRUE){modelfit <- cbind(chisq, df, AIC, CFI, SRMR)} else {modelfit <- cbind(chisq, df, AIC, SRMR)}
  ...
  if(class(bread2$value)[1] == "matrix" & check == 2){  
    ...
    colnames(results)=c("lhs","op","rhs","Unstand_Est","Unstand_SE","Stand_Est","Stand_SE", "Stand_All")
    
    ##name model fit columns
    if(CFIcalc == TRUE){colnames(modelfit)=c("chisq","df","AIC","CFI","SRMR")}else{colnames(modelfit)=c("chisq","df","AIC","SRMR")}
    ...
    if(!(is.character(modelfit$chisq)) & !(is.factor(modelfit$chisq))) {
      ...
      modelfit$chisq <- ifelse(modelfit$df == 0, is.na(modelfit$chisq), modelfit$chisq)  
      modelfit$AIC <- ifelse(modelfit$df == 0, is.na(modelfit$AIC), modelfit$AIC)  
      modelfit$p_chisq <- ifelse(modelfit$df == 0, is.na(modelfit$p_chisq), modelfit$p_chisq)
      modelfit$SRMR <- ifelse(modelfit$df == 0, is.na(modelfit$SRMR), modelfit$SRMR)
      if(CFIcalc == TRUE){
        order <- c(1,2,6,3,4,5)
        ...
        modelfit$CFI <- ifelse(modelfit$df == 0, is.na(modelfit$CFI), modelfit$CFI)
      } else {
        order <- c(1,2,5,3,4)
        modelfit <- modelfit[, order]
      }
    ...
    if(imp_cov == TRUE){
      ...
      names(resid_cov) <-
        c(
          ...
          "Residual Covariance Matrix (i.0., Calculated_Observed_Cov - Model_Implied_Cov)"
        )
