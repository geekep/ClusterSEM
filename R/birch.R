#' Create a clustering feature tree using the BIRCH algorithm
#' @description Function to create a clustering feature tree using the BIRCH algorithm
#'
#' @param x Input to BirchCF function in dataframe format
#' @param Type default to 'df'
#' @param branchingfactor default to 4
#' @param threshold default to 0.15
#'
#' @return A hierarchical clustering feature tree
#' 
#' @export
BirchCF <- function(x, Type = 'df', branchingfactor = 4, threshold = 0.15)
{
  ...
  if(is.data.frame(x))
  {
    ...
    DataframeProcessing(as.data.frame(x), threshold, branchingfactor)
    y = mainlist;
  }
...
MakeaCFTree <- function (x, pagesize, threshold, i)
{
  ...
  if(length(mainlist) == 0 )
  {
    ...
    mainlist[[length(mainlist)]][[1]] <<- list(N = n,
                                               LS = rowvector,
                                               SS = rowvector * rowvector,
                                               RN = rootnode,
                                               CN = childnode,
                                               CNT = 0,
                                               CI = array(i))
  ...
  else
  {
    ...
    if(radius>threshold)
    {
      ...
      if(count == pagesize)
      {
        ...
        mainlist[[depth]][[length(mainlist[[depth]])+1]] <<- list(N = 1,
                                                                  LS = rowvector,
                                                                  SS = rowvector * rowvector,
                                                                  CN = temporarycf$CN,
                                                                  CNT = 0,
    ...
    else
    {
      ...
      templist = list(N = f$N + 1,
                      LS = f$LS + rowvector,
                      SS = f$SS + (rowvector * rowvector),
                      CN = f$CN,
                      CNT = f$CNT,
                      RN = f$RN,
                      CI = append(f$CI, i));
...
calculatenearestnode <- function (parentnode, rowvector, depth)
{
  ...
  if(x[[index]]$CNT != 0)
  {
    #recursive function to travel deep into the tree
...
rearrange <- function(depth, index)
{
  ...
  for(i in c(cf1list))
  {
    ...
    else
    {
      ...
      CF1index = list(N = CF1index$N + z$N,
                      LS = CF1index$LS + z$LS,
                      SS = CF1index$SS + z$SS,
                      CNT = Mainchild$CNT,
                      CN = Mainchild$CN,
  ...
  for(i in c(cf2list))
  {
    ...
    {
      z = mainlist[[depth-1]][[indexvector[i]]]
      CF2index = list(N = CF2index$N + z$N,
                      LS = CF2index$LS + z$LS,
                      SS = CF2index$SS + z$SS,
                      CNT = Mainchild$CNT,
                      CN = Mainchild$CN,
...
Fit.CFTree <- function (Type, element, nClusters, nStart, iter.max = 100, method="complete")
{
  array1 = array();
  array2 = array();
  # matrix for clustering
  centroid = matrix(nrow = length(element),ncol = length(element[[1]]$LS))
  ...
  if(Type =='kmeans'){
    ...
    kclust = stats::kmeans(x = centroid, centers = nClusters, nstart = nStart, iter.max = 100)
    cluster = kclust$cluster
  }
  else if (Type == 'hclust'){
    print("Hierarchical clustering")
    hcluster = stats::hclust(stats::dist(centroid), method)
    cluster = stats::cutree(hcluster, nClusters)
  }
  for (i in c(1:length(element)))
  {
