#' Run common factor model on genetic covariance S and sampling covariance matrix V
#' @description Function to run a common factor model based on output from multivariate LDSC/HDL/metaCCA
#'
#' @param covstruc Output from the multivariate LDSC/HDL/metaCCA function
#' @param estimation Options are either Diagonally Weighted Least Squares ("DWLS"; the default) or Maximum Likelihood ("ML")
#'
#' @return Common factor structure list
#' 
#' @export
commonfactor <- function(covstruc, estimation="DWLS"){
  ...
  ##k = number of phenotype in dataset (i.e., number of columns in LD portion of S matrix)
  k <- ncol(S_LD)
  ...
  if(k == 2){
    stop("Their are only 2 variables in the genetic covariance matrix so the common factor model will be under-identified (df = -1). 
         You can either specify a common factor model with constrained factor loadings with the usermodel function or rerun ldsc with at least one additional variable.")
  }       
  ...
  S_LDb <- S_LD
  smooth1 <-
    ifelse(eigen(S_LD)$values[ks] <= 0, S_LD <-
  ...
  V_LDb <- V_LD
  smooth2 <-
    ifelse(eigen(V_LD)$values[kv] <= 0, V_LD <-
  ...
  empty$warning$message[1] <-
    ifelse(
      is.null(empty$warning$message),
      empty$warning$message[1] <- 0,
      empty$warning$message[1]
    )
  
  if (class(empty$value)[1] == "simpleError" |
      grepl("solution has NOT",  as.character(empty$warning)) == TRUE) {
    print(
      "The common factor initially failed to converge. A lower bound of 0 on residual variances has been added to try and troubleshoot this."
    )
  ...
  if (class(empty$value)[1] == "simpleError") {
    print(
      "The common factor model failed to converge on a solution. Please try specifying an alternative model using the usermodel function."
    )
  }
  ...
  if (as.character(testCFI$warning$message)[1] != "lavaan WARNING: model has NOT converged!") {
    ...
    if (as.character(testCFI2$warning$message)[1] != "lavaan WARNING: model has NOT converged!") {
      #pull the delta matrix (this doesn't depend on N)
      S2.delt_Q_CFI <- lavaan::lavInspect(ModelQ_Results_CFI, "delta")
      
      ##weight matrix from stage 2
      S2.W_Q_CFI <- lavaan::lavInspect(ModelQ_Results_CFI, "WLS.V")
  ...
  ##df of user model
  df <- lavaan::lavInspect(Model1_Results, "fit")["df"]
  ...
  #re-scale the SE by the same multiples that the S matrix was re-scaled by
  ...
  delt_stand <- lavaan::lavInspect(Fit_stand, "delta")
  W_stand <- lavaan::lavInspect(Fit_stand, "WLS.V")
  ...
  unstand <-
    data.frame(lavaan::inspect(Model1_Results, "list")[, c(2:4, 8, 14)])
  unstand <- subset(unstand, unstand$free != 0)
  unstand$free <- NULL
  
  stand <- data.frame(lavaan::inspect(Fit_stand, "list")[, c(8, 14)])
  stand <- subset(stand, stand$free != 0)
  stand$free <- NULL
  
  chisq <- Q
  df <- lavaan::lavInspect(Model1_Results, "fit")["df"]
  AIC <- (Q + 2 * lavaan::lavInspect(Model1_Results, "fit")["npar"])
  SRMR <- lavaan::lavInspect(Model1_Results, "fit")["srmr"]
  ...
  modelfit$p_chisq <-
    ifelse(modelfit$df == 0, is.na(modelfit$p_chisq), modelfit$p_chisq)
  ...
  results$p_value <-
    2 * pnorm(abs(
      as.numeric(results$Unstandardized_Estimate) / as.numeric(results$Unstandardized_SE)
    ), lower.tail = FALSE)
  results$p_value <-
