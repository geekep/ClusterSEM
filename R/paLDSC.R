...
# from the LDSC output), or the genetic covariance and unstandardized sampling distribution matrices from the LDSC output (S and V matrices, respectively, which can be obtained by setting stand = TRUE in the LDSC function). 
#
...
#  - diag: defines whether diagonallized PA should be conducted (FALSE by default). Diagonallized PA assumes uncorrelated sampling errors for the simulated null correlation matrices, as would happen if the data were pure noise.
#  Thus, if diag = TRUE, the function will only use the diagonal of the sampling distribution V (i.e., the sampling variances or the phenotypes in the original genetic correlation matrix) to introduce variation in the simulated null correlation matrices.
# If diag = FALSE (by default), the whole multivariate sampling distribution LDSC V matrix will be used to simulate the null correlation matrices, thus assuming correlated sampling errors.
#  - fa: defines whether the eigenvalues should also be computed from a common factor solution from a explotory factor analysis of n factors (by default 1) using the factor method fm (by default minimum residual). By default this argument is set to FALSE, since there is some degree of variation on the factor analysis eigenvalues depending on the number of factors extracted.
...
#' @export
paLDSC <- function(S = S, V = V, r = NULL, p = NULL, save.pdf = F, diag = F, fa = F, fm = NULL, nfactors = NULL) {
  ...
  for (i in 1:r) {
    ...
    EIG[, i] = eigen(Sample_null_M)$values
