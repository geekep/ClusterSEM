...
hdl <- function(traits, sample.prev = NA, population.prev = NA, trait.names = NULL,
  ...
  for (j in 1:n.traits) {
    for (d in j:n.traits) {
      ...
      if (j == d) {
        chi1 <- traits[j]
        gwas.df <- as.data.frame(suppressMessages(
          readr::read_delim(chi1, "\t", escape_double = FALSE, trim_ws = TRUE, progress = F)
          ))
        ...
        if (!("Z" %in% colnames(gwas.df))) {
          if (("b" %in% colnames(gwas.df)) && ("se" %in% colnames(gwas.df))) {gwas.df$Z <- gwas.df$b / gwas.df$se}
          else {stop("Z is not available, meanwhile either b or se is missing. Please check.")}
        }
        ...
        if (k1 < length(snps.name.list) * 0.99) {cat("Warning: More than 1% SNPs in reference panel are missed in the GWAS. This may generate bias in estimation. Please make sure that you are using correct reference panel. \n")}
        ...
        for (chr in 1:22) {
          ...
          for (piece in 1:k) {
            ...
            opt <- optim(c(h11.wls[2], 1), llfun, N = N1, Nref = Nref, lam = lam,
                         bstar = bstar1, M = M, lim = exp(-18), method = "L-BFGS-B",
        ...
        if (method == "jackknife") {
          ...
          opt <- optim(c(sum(HDL11.df[, 1]), 1), llfun, N = N1, Nref = Nref,
                       lam = unlist(lam.v), bstar = unlist(bstar1.v),
                       M = M.ref, lim = exp(-18), method = 'L-BFGS-B',
                       lower = c(0, 0), upper = c(1, 10))
          h11.hdl <- opt$par
          ...
          for (i in 1:length(lam.v)) {
            opt <- optim(h11.hdl, llfun, N = N1, Nref = Nref,
                         lam = unlist(lam.v[-i]), bstar = unlist(bstar1.v[-i]),
                         M = M.ref, lim = exp(-18), method = 'L-BFGS-B',
                         lower = c(0, 0), upper = c(1, 10))
        ...
        if (method == "piecewise") {
          ...
          for (i in 1:num.pieces) {V.hold[i, s] <- (num.pieces / (num.pieces - 1)) * sum(HDL11.df[-i, 1])}
        }
        s <- s + 1
      }
      ...
      if (j != d) {
        chi1 <- traits[j]
        chi2 <- traits[d]
        gwas1.df <- as.data.frame(suppressMessages(
            readr::read_delim(chi1, "\t", escape_double = FALSE, trim_ws = TRUE, progress = F)
          ))
        gwas2.df <- as.data.frame(suppressMessages(
            readr::read_delim(chi2, "\t", escape_double = FALSE, trim_ws = TRUE, progress = F)
          ))
        ...
        rho12 <- suppressWarnings(
            dplyr::inner_join(gwas1.df , gwas2.df , by = "SNP") %>% dplyr::summarise(x = cor(Z.x, Z.y, use = "complete.obs")) %>% unlist
            )
        ...
        if (method == "jackknife") {
          ...
          opt <- optim(c(sum(HDL11.df[, 1]), 1), llfun, N = N1, Nref = Nref,
                       ...
                       lower = c(0, 0), upper = c(1, 10))
          h11.hdl <- opt$par
          opt <- optim(c(sum(HDL22.df[, 1]), 1), llfun, N = N2, Nref = Nref,
                       lam = unlist(lam.v), bstar = unlist(bstar2.v),
                       M = M.ref, lim = exp(-18), method = 'L-BFGS-B',
                       lower = c(0, 0), upper = c(1, 10)
            )
          h22.hdl <- opt$par
          opt <- optim(c(sum(HDL12.df[, 1]), rho12), llfun.gcov.part.2, h11 = h11.hdl, h22 = h22.hdl,
                       ...
                       bstar1 = unlist(bstar1.v), bstar2 = unlist(bstar2.v), lim = exp(-18),
          ...
          if (opt$convergence != 0) {
            ...
            while (opt$convergence != 0) {
              ...
              opt <- optim(c(starting.value, rho12), llfun.gcov.part.2, h11 = h11.hdl, h22 = h22.hdl,
                           ...
                           method = 'L-BFGS-B', lower = c(-1, -10), upper = c(1, 10))
              k <- k + 1
              if (k > length(starting.value.v)) {stop("Algorithm failed to converge after trying different initial values. \n")}
            }
          }
          h12.hdl <- opt$par
          ...
          for (i in 1:length(lam.v)) {
            ...
            opt <- optim(h22.hdl, llfun, N = N2, Nref = Nref,
                         lam = unlist(lam.v[-i]), bstar = unlist(bstar2.v[-i]),
                         M = M.ref, lim = exp(-18), method = 'L-BFGS-B',
                         lower = c(0, 0), upper = c(1, 10))
            h22.hdl.jackknife <- opt$par
            
            opt <- optim(h12.hdl, llfun.gcov.part.2, h11 = h11.hdl, h22 = h22.hdl, rho12 = rho12,
                         M = M.ref, N1 = N1, N2 = N2, N0 = N0, Nref = Nref,
                         lam0 = unlist(lam.v[-i]), lam1 = unlist(lam.v[-i]), lam2 = unlist(lam.v[-i]),
                         bstar1 = unlist(bstar1.v[-i]), bstar2 = unlist(bstar2.v[-i]), lim = exp(-18),
                         method = 'L-BFGS-B', lower = c(-1, -10), upper = c(1, 10))
        ...
        if (method == "piecewise") {
          ...
          S[j, d] <- S[d, j] <- gen.cov
          for (i in 1:num.pieces) {V.hold[i, s] <- (num.pieces / (num.pieces - 1)) * sum(HDL12.df[-i, 1])}
        }
        s <- s + 1
      }
    }
